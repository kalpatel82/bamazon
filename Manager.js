...
function addToInventory(){
  ...
  inquirer.prompt([{
    ...
    validate: function(value){
      ...
      var currentQty;
      for(var i=0; i<res.length; i++){
        if(res[i].prodName === ans.product){
          currentQty = res[i].stockQuant;
        }
      }
      connection.query('UPDATE products SET ? WHERE ?', [
        {stockQuant: currentQty + parseInt(ans.qty)},
        {prodName: ans.product}
        ], function(err, res){
          if(err) throw err;
